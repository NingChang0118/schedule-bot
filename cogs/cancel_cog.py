...
class CancelCog(commands.Cog):
    ...
    @app_commands.command(
        name="s6砍班",
        description="取消自己指定時段或整天的 S6 報班"
    )
    async def cancel_s6_schedule(
        self,
        interaction: discord.Interaction,
        car: str,
        date: str,
        ...
        removed_times_for_s6_check = [
            record.split("：")[0]
            for record in removed_records
        ]

        for target_time in removed_times_for_s6_check:
            target_row = get_row_by_time(
                schedule,
                target_time
            )
